...
public class ImagePaletteModule: Module {
  ...
  private let workQueue = DispatchQueue(
    label: "com.imagepalette.work",
    qos: .userInitiated
  )

  #if IMAGE_PALETTE_WORKER_THREAD
  private static let THREAD_TAG = "worker"
  #else
  private static let THREAD_TAG = "main"
  #endif

  private static let logger = Logger(subsystem: "com.imagepalette", category: THREAD_TAG)

  private func log(_ msg: String) {
    let line = "[ImagePalette/\(Self.THREAD_TAG)] \(msg)"
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("getDominantColors") { (uri: String) -> [String: Any] in
      ...
      guard let image = ImageDecoder.decode(
        ...
        return self.respond(swatches: [], config: cfg)
      }
